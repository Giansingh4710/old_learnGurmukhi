...
function show_ans_week() {
  ...
  document.getElementById("day_of_week_ans").innerHTML =
    `${engVer} : ${punjVer}`;
}

const engToPunjNumbers1_100 = {
  1: ["One", "੧", "Ikk", "ਇੱਕ"],
  2: ["Two", "੨", "Do", "ਦੋ"],
  3: ["Three", "੩", "Tin", "ਤਿੰਨ"],
  4: ["Four", "੪", "Char", "ਚਾਰ"],
...
function putNumbersInTable() {
  const tableBody = document.querySelector("#allNumsTable tbody");
  tableBody.innerHTML = ""; // Clear existing rows

  for (const [key, value] of Object.entries(engToPunjNumbers1_100)) {
    const row = document.createElement("tr");

    const numberCell = document.createElement("td");
    numberCell.textContent = key;
    row.appendChild(numberCell);

    const englishCell = document.createElement("td");
    englishCell.textContent = value[0];
    row.appendChild(englishCell);

    const numeralsCell = document.createElement("td");
    numeralsCell.textContent = value[1];
    row.appendChild(numeralsCell);

    const transliterationCell = document.createElement("td");
    transliterationCell.textContent = value[2];
    row.appendChild(transliterationCell);

    const gurmukhiCell = document.createElement("td");
    gurmukhiCell.textContent = value[3];
...
document.addEventListener("DOMContentLoaded", putNumbersInTable);
...
function show_ans_numbers(num) {
  if (num === "") {
    ...
    num = Object.keys(engToPunjNumbers1_100)[Math.floor(Math.random() * 100)];
  }
  num = num.replace(/,/g, "");

  let punjVer = engToPunjNumbers1_100[num];
  if (!(num in engToPunjNumbers1_100)) {
...
function num_pronunciation(num) {
  ...
  if (num in NUMS) {
    ...
    pun_num =
      ...
      NUMS["100"] +
      " " +
      num_pronunciation(num.substring(1));
